  ...
  public List<EmployeeDepartment> read(String dir) {
    ...
    try (Scanner s = new Scanner(file)) {
      while (s.hasNextLine()) {
        ...
        LocalDate fecha = LocalDate.of(Integer.parseInt(fechaString[0]), Integer.parseInt(fechaString[1]),
            Integer.parseInt(fechaString[2]));
        EmployeeDepartment ed = new EmployeeDepartment(Integer.valueOf(data[0]), data[1], data[2], data[3], data[4],
            fecha);
        lista.add(ed);
      }
    } catch (FileNotFoundException ex) {
      Logger.getLogger(EmployeeDptIO.class.getName()).log(Level.SEVERE, null, ex);
    }
    return lista;
  }
